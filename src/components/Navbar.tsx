...
export default function Navbar({ activeSection, toggles = { experience: true, projects: true }, sectionOrder }: NavbarProps) {
  ...
  const links = [
    ...(toggles.experience !== false ? [{ label: "experience", target: "services", key: "experience" }] : []),
    { label: "about", target: "about", key: "about" },
    ...(toggles.projects !== false ? [{ label: "projects", target: "projects", key: "projects" }] : []),
  ];
  ...
  return (
    <>
      <header
        id="app-header"
        className={`fixed top-0 left-0 z-40 w-full transition-all duration-300 ${scrolled
            ? "bg-primary-dark/60 backdrop-blur-2xl py-3 border-b border-transparent [border-image:linear-gradient(to_right,transparent,rgba(255,20,147,0.6),transparent)_1]"
          ...
          <nav className="hidden md:flex items-center gap-8 text-xs font-mono tracking-widest text-neutral-300">
            {links.map((link) => (
              <button
                ...
                className={`uppercase border-b py-1 transition-all duration-300 ${activeSection === link.target
