...
export default function Contact() {
  ...
  const [contactEmail, setContactEmail] = useState("chitturianjana@gmail.com");
  ...
  // Ensure links always have a protocol so they don't resolve as relative paths
        ...
        if (Array.isArray(data)) {
          const updated = SOCIAL_LINKS.map(link => {
            if (link.name === "LinkedIn") {
              ...
              if (found && found.value) return { ...link, url: normalizeUrl(found.value) };
            }
            if (link.name === "GitHub") {
            ...
            if (link.name === "E-Mail") {
              const found = data.find(item => item.key === 'hero_email_url' || item.key === 'contact_email');
              if (found && found.value) return { ...link, url: found.value.startsWith('mailto:') ? found.value : `mailto:${found.value}` };
            }
            return link;
          });
          ...
          setContactEmail(getVal('hero_email_url', getVal('contact_email', 'chitturianjana@gmail.com')));
  ...
  // Parallax states for SHREYA signature
  ...
  const handleFormSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "Message transmission failed.");
      }
      ...
      setErrorMessage(err.message || `Could not transmit inquiry. Please email ${contactEmail} directly.`);
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <section
      id="contact"
      className="relative bg-primary-dark pt-16 md:pt-24 pb-8 border-t border-white/5 cursor-default overflow-hidden min-h-screen flex flex-col justify-center"
    >
      <style>{`
        @keyframes btnSweep {
        ...
        .giant-signature-shimmer {
          background: linear-gradient(
            90deg,
            rgba(255, 77, 148, 0.05) 0%,
            rgba(255, 77, 148, 0.75) 50%,
            rgba(255, 77, 148, 0.05) 100%
          );
        ...
        @keyframes signaturePulse {
          0%, 100% {
            filter: drop-shadow(0 0 10px rgba(255, 77, 148, 0.06)) drop-shadow(0 0 2px rgba(255, 77, 148, 0.15));
            -webkit-text-stroke: 1.5px rgba(255, 77, 148, 0.25);
          }
          50% {
            filter: drop-shadow(0 0 20px rgba(255, 77, 148, 0.15)) drop-shadow(0 0 5px rgba(255, 77, 148, 0.3));
            -webkit-text-stroke: 1.5px rgba(255, 77, 148, 0.45);
          }
        }
        ...
        .giant-signature-outline {
          ...
          -webkit-text-stroke: 1.5px rgba(255, 77, 148, 0.25);
          text-shadow: 
            1px 1px 0px rgba(255, 77, 148, 0.05),
            2px 2px 0px rgba(255, 77, 148, 0.03),
            3px 3px 6px rgba(255, 77, 148, 0.1);
        ...
        .giant-signature-outline:hover {
          -webkit-text-stroke: 1.5px rgba(255, 77, 148, 0.7);
          text-shadow: 
            1px 1px 0px rgba(255, 77, 148, 0.1),
            2px 2px 0px rgba(255, 77, 148, 0.08),
            3px 3px 12px rgba(255, 77, 148, 0.25);
          filter: drop-shadow(0 0 25px rgba(255, 77, 148, 0.25)) drop-shadow(0 0 6px rgba(255, 77, 148, 0.5));
          animation: none;
        }
      `}</style>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
        {[...Array(8)].map((_, i) => (
          <motion.div
            ...
            className="absolute w-1 h-1 bg-[#ff4d94] rounded-full blur-[0.5px]"
          />
        ))}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-4 flex flex-col space-y-8 text-left">
            <motion.div
              ...
              className="space-y-3"
            >
              <span className="font-sans text-[10px] text-[#ff4d94] tracking-widest uppercase block font-bold">{consultationsLabel}</span>
          ...
          <motion.div
            ...
            className="lg:col-span-8 bg-white/2 border border-white/10 backdrop-blur-xl p-8 md:p-10 rounded-2xl relative lg:mt-14 shadow-[0_0_50px_rgba(255, 77, 148,0.03),inset_0_1px_1px_rgba(255,255,255,0.05)] hover:border-[#ff4d94]/25 hover:shadow-[0_0_50px_rgba(255, 77, 148,0.08),inset_0_1px_1px_rgba(255,255,255,0.08)] transition-all duration-500"
          >
            {submitted ? (
              <div className="py-16 flex flex-col justify-center items-center text-center space-y-4">
                <motion.div
                  ...
                  className="h-16 w-16 rounded-full bg-[#ff4d94]/10 flex items-center justify-center border border-[#ff4d94]/30 shadow-[0_0_20px_rgba(255, 77, 148,0.2)]"
                >
                  <CheckCircle2 className="h-8 w-8 text-[#ff4d94]" />
                </motion.div>
              ...
              <>
                ...
                <div className="mb-8 text-left space-y-2">
                  <span className="font-syne text-[10px] text-[#ff4d94] tracking-[0.25em] uppercase font-black block">// INITIATE CONTACT</span>
                ...
                <form onSubmit={handleFormSubmit} className="space-y-8 text-left">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="relative pt-2 w-full">
                      <input
                        ...
                        placeholder="Name"
                        className="peer w-full bg-white/2 border border-neutral-800 hover:border-neutral-700 focus:border-[#ff4d94] rounded-xl px-4 py-3.5 text-xs text-white placeholder-transparent focus:outline-none focus:shadow-[0_0_15px_rgba(255, 77, 148,0.2)] transition-all duration-300 ease-out"
                      />
                      <label
                        htmlFor="form-name"
                        className={`absolute left-4 select-none pointer-events-none transition-all duration-300 origin-left px-1.5
                        ${(name || nameFocus)
                            ? "top-0 text-[10px] text-[#ff4d94] font-bold uppercase tracking-widest bg-primary-dark -translate-y-1/2 scale-90"
                  ...
                  <div className="relative pt-2 w-full">
                    <textarea
                      ...
                      placeholder="Message"
                      className="peer w-full bg-white/2 border border-neutral-800 hover:border-neutral-700 focus:border-[#ff4d94] rounded-xl p-4 text-xs text-white placeholder-transparent focus:outline-none focus:shadow-[0_0_15px_rgba(255, 77, 148,0.2)] transition-all duration-300 ease-out resize-none"
                    />
                    <label
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className={`group relative overflow-hidden w-full font-syne text-[13px] uppercase tracking-widest font-black py-5 rounded-xl transition-all flex items-center justify-center gap-2 focus:outline-none border select-none ${sending ? "opacity-75 cursor-not-allowed" : "cursor-pointer"
                      } ${submitted
                        ? "bg-[#ff4d94]/10 border-[#ff4d94] text-[#ff4d94] shadow-[0_0_20px_rgba(255, 77, 148,0.15)]"
                        : "bg-white text-black border-transparent hover:bg-neutral-200"
                      }`}
                  >
                    {/* Subtle sweep overlay */}
                    {!submitted && !sending && (
                      <div className="absolute inset-0 bg-linear-to-r from-transparent via-[#ff4d94]/25 to-transparent pointer-events-none btn-sweep-overlay w-[100px] z-0" />
                    )}
        ...
        <motion.div
          ...
          className="mt-14 border border-[#ff4d94]/25 bg-white/3 backdrop-blur-xl p-6 md:p-8 rounded-2xl relative shadow-lg hover:-translate-y-1 transition-all duration-300 w-full"
        >
          {/* Gold light corner reflection highlight */}
          <div className="absolute top-0 right-0 w-24 h-24 bg-[#ff4d94]/5 rounded-bl-full pointer-events-none filter blur-md" />

          {/* Status Header */}
          <div className="flex items-center gap-2.5 pb-4 border-b border-white/5 mb-6">
            <span className="relative flex h-3 w-3">
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${availabilityColor === 'green' ? 'bg-green-400' :
                  availabilityColor === 'red' ? 'bg-red-400' :
                    availabilityColor === 'yellow' ? 'bg-yellow-400' :
                      'bg-blue-400'
                }`}></span>
              <span className={`relative inline-flex rounded-full h-3 w-3 ${availabilityColor === 'green' ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.8)]' :
            ...
            <span className={`text-[10px] font-syne font-black tracking-widest uppercase ${availabilityColor === 'green' ? 'text-green-400' :
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start text-xs font-sans">
            ...
            <div className="md:col-span-7 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="flex gap-2">
                  <Clock className="w-4 h-4 text-[#ff4d94]/70 shrink-0 mt-0.5" />
                  <div>
                ...
                <div className="flex gap-2">
                  <Globe className="w-4 h-4 text-[#ff4d94]/70 shrink-0 mt-0.5" />
                  <div>
              ...
              <div className="border-t border-white/5 pt-4 flex gap-2">
                <Briefcase className="w-4 h-4 text-[#ff4d94]/70 shrink-0 mt-0.5" />
                <div className="w-full">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {preferredRoles.map((role, idx) => (
                      <span key={idx} className="px-2.5 py-1 rounded-md bg-white/5 border border-white/5 text-[9px] text-neutral-300 font-medium hover:border-[#ff4d94]/30 hover:text-white transition-all">{role}</span>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="md:col-span-5 md:border-l md:border-white/5 md:pl-8 space-y-4">
              ...
              <a
                ...
                className="group flex items-center gap-3 hover:text-[#ff4d94] transition-colors font-medium text-xs text-neutral-300"
              >
                <div className="h-7 w-7 rounded-full bg-white/5 flex items-center justify-center border border-white/5 group-hover:border-[#ff4d94]/25 group-hover:bg-[#ff4d94]/5 transition-colors shrink-0">
                  <Mail className="h-3.5 w-3.5 text-neutral-500 group-hover:text-[#ff4d94]" />
                </div>
              ...
              >
                <div className="h-7 w-7 rounded-full bg-white/5 flex items-center justify-center border border-white/5 group-hover:border-[#ff4d94]/25 group-hover:bg-[#ff4d94]/5 transition-colors shrink-0">
                  <Phone className="h-3.5 w-3.5 text-neutral-500 group-hover:text-[#ff4d94]" />
                </div>
          ...
          <div className="flex flex-wrap justify-center items-center gap-10 md:gap-16 w-full">
            {socialLinks.map((link) => (
              <a
                ...
                className="group relative font-sans text-sm sm:text-[15px] uppercase tracking-[0.18em] hover:tracking-[0.28em] text-neutral-500 hover:text-[#ff4d94] active:text-[#ff4d94] transition-all duration-300 ease-out flex flex-col items-center py-2.5 hover:-translate-y-1.5 select-none font-bold"
              >
                <span className="group-hover:drop-shadow-[0_0_8px_rgba(255, 77, 148,0.6)] transition-all duration-300">
                  {link.name}
                </span>
                <span className="absolute bottom-0 w-0 h-[1.5px] bg-[#ff4d94] group-hover:w-full transition-all duration-300 shadow-[0_0_8px_rgba(255, 77, 148,0.7)]" />
              </a>
            ))}
          </div>
        ...
        >
          ...
          <div className="w-32 h-px bg-linear-to-r from-transparent via-[#ff4d94]/35 to-transparent mx-auto mb-10" />
        ...
        <div className="relative mt-6 py-8 md:py-12 overflow-visible flex justify-center w-full select-none cursor-default">
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden z-0 flex items-center justify-center">
            <motion.div
              ...
              className="absolute w-[150%] h-[180px] bg-[linear-gradient(90deg,transparent_20%,rgba(255, 77, 148,0.06)_50%,transparent_80%)] -skew-x-12 blur-3xl"
            />
          </div>

          {/* Mega outline Text watermark branding trademark (Screen 12 inspired) */}
          <div className="mt-[-75px] relative select-none pointer-events-none text-center">
            <span
              className="font-syne font-black text-[12vw] select-none pointer-events-none uppercase"
              style={{
        ...
        >
          <span className="font-syne text-[11px] sm:text-xs font-black uppercase tracking-[0.25em] text-[#ff4d94]/85">
            Thanks for visiting.
          </span>
        ...
        <div className="mt-8 border-t border-white/5 pt-6 flex flex-col sm:flex-row justify-between items-center text-[9px] font-mono text-neutral-600 gap-3">
          ...
          <a href="#/admin" className="hover:text-[#ff4d94] transition-colors cursor-pointer uppercase tracking-widest border border-white/5 px-2 py-1 rounded bg-black/20 hover:bg-[#ff4d94]/10">
            Admin Login
          </a>
