...
const Modal = ({ isOpen, onClose, title, children }: any) => {
  ...
  return (
    <div className="fixed top-0 left-72 right-0 bottom-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/70 backdrop-blur-md animate-fade-in">
      <div className="bg-neutral-900 border border-white/10 rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh] animate-slide-up relative">
        {/* Subtle top glow */}
        <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-transparent via-[#ff4d94]/50 to-transparent opacity-50" />
    ...
    <div className="flex flex-col space-y-2.5 bg-black/20 p-4 border border-white/5 rounded-2xl">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="text-[10px] font-mono uppercase tracking-widest text-[#ff4d94] hover:text-white flex items-center gap-1 transition-colors bg-white/5 px-2.5 py-1.5 rounded-lg border border-white/5 hover:border-white/10"
        >
      ...
      {value.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto custom-scrollbar pr-1">
          {value.map((item: string, idx: number) => (
            <div key={idx} className="flex items-center gap-2 bg-black/40 border border-white/5 rounded-xl p-1.5 focus-within:border-[#ff4d94]/30 transition-colors">
              <input
    ...
    <div className="flex flex-col space-y-2.5 bg-black/20 p-4 border border-white/5 rounded-2xl">
        ...
        {allOptions.map((option: string) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2.5 p-2.5 rounded-xl border text-xs font-medium transition-all text-left ${isSelected
                ? 'bg-[#ff4d94]/10 border-[#ff4d94]/30 text-white shadow-[0_0_15px_rgba(255, 77, 148,0.15)]'
                : 'bg-black/40 border-white/5 text-neutral-400 hover:border-white/20 hover:text-white'
                }`}
            >
              <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all shrink-0 ${isSelected ? 'bg-[#ff4d94] border-[#ff4d94]' : 'bg-black/50 border-white/20'}`}>
      ...
      <form onSubmit={handleAddCustom} className="flex gap-2 mt-2 pt-2 border-t border-white/5">
        <input
          ...
          className="bg-black/50 border border-white/10 rounded-xl px-3 py-1.5 text-xs text-white focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none grow"
        />
        <button
          type="submit"
          className="text-xs font-mono uppercase tracking-wider bg-[#ff4d94] hover:bg-[#e5bf45] text-black px-3 py-1.5 rounded-xl font-bold transition-all shrink-0"
        >
          Add Custom
        </button>
      </form>
...
const TextInput = ({ label, value, onChange, placeholder, required = false, type = 'text' }: any) => (
  <div className="flex flex-col">
    ...
    <input
      type={type}
      className="bg-black/50 border border-white/10 p-3 rounded-xl text-white text-sm focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none transition-all"
      placeholder={placeholder}
      value={value}
...
const TextAreaInput = ({ label, value, onChange, placeholder, required = false, rows = 3 }: any) => (
  <div className="flex flex-col">
    ...
    <textarea
      rows={rows}
      className="bg-black/50 border border-white/10 p-3 rounded-xl text-white text-sm focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none transition-all resize-y"
      placeholder={placeholder}
      value={value}
...
const SelectInput = ({ label, value, onChange, options, required = false }: any) => (
  <div className="flex flex-col">
    <label className="text-[10px] text-neutral-400 mb-1.5 font-mono uppercase tracking-wider pl-1">{label}</label>
    <select
      className="bg-black/50 border border-white/10 p-3 rounded-xl text-white text-sm focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none transition-all cursor-pointer"
      value={value}
      onChange={e => onChange(e.target.value)}
      required={required}
    >
...
const CheckboxInput = ({ label, checked, onChange }: any) => (
  <div className="flex items-center space-x-3 mt-4 p-4 bg-black/30 border border-white/5 rounded-xl hover:bg-white/5 transition-colors cursor-pointer" onClick={() => onChange(!checked)}>
    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${checked ? 'bg-[#ff4d94] border-[#ff4d94]' : 'bg-black/50 border-white/20'}`}>
...
const PageHeader = ({ title, onAdd, addText }: any) => (
  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    <div>
      ...
      <div className="h-1 w-12 bg-[#ff4d94] mt-3 rounded-full opacity-80" />
    </div>
    {onAdd && (
      <button
        onClick={onAdd}
        className="flex items-center gap-2 bg-[#ff4d94] text-black px-5 py-2.5 rounded-xl font-bold hover:bg-[#ff4d94]/90 hover:scale-105 hover:shadow-[0_0_20px_rgba(255, 77, 148,0.3)] transition-all active:scale-95"
      >
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingId ? 'Edit Project' : 'Create Project'}>
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="flex flex-col">
            ...
            <textarea className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white text-sm h-32 focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none transition-all custom-scrollbar" placeholder="Detailed description..." value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} required />
          </div>
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-white/5 mt-6">
            ...
            <button type="submit" className="bg-[#ff4d94] text-black px-6 py-2.5 rounded-xl font-bold hover:bg-[#e5bf45] transition-colors">{editingId ? 'Save Changes' : 'Publish Project'}</button>
          </div>
        </form>
      </Modal>
        ...
        {items.map((p: any, index: number) => {
          ...
          return (
            <div
              ...
              className={`group relative bg-neutral-900/40 backdrop-blur-sm border transition-all duration-300 hover:shadow-xl flex flex-col h-full rounded-2xl ${isDragging
                ? 'opacity-40 border-dashed border-[#ff4d94]/50 scale-95'
            ...
            >
              <div className="flex justify-between items-center p-5 pb-2">
                <div className="flex items-center gap-3">
                  ...
                  {p.featured && <span className="bg-[#ff4d94]/10 border border-[#ff4d94]/20 text-[#ff4d94] text-[10px] px-2 py-1 rounded-md uppercase font-bold tracking-wider">Featured</span>}
                </div>
    ...
    <div>
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingId ? 'Edit Experience' : 'Add Experience'}>
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-white/5 mt-6">
            <button type="button" onClick={closeModal} className="px-5 py-2.5 rounded-xl font-medium text-neutral-400 hover:text-white hover:bg-white/5 transition-colors">Cancel</button>
            <button type="submit" className="bg-[#ff4d94] text-black px-6 py-2.5 rounded-xl font-bold hover:bg-[#e5bf45] transition-colors">{editingId ? 'Save' : 'Add Role'}</button>
          </div>
        </form>
      </Modal>
        ...
        {items.map((p: any, index: number) => {
          ...
          return (
            <div
              ...
              onDragOver={(e) => handleDragOver(e, index)}
              onDragEnd={handleDragEnd}
              className={`group bg-neutral-900/40 backdrop-blur-sm border transition-all duration-300 flex justify-between items-center p-5 rounded-2xl ${isDragging
                ? 'opacity-40 border-dashed border-[#ff4d94]/50 scale-98'
                : 'border-white/5 hover:border-white/20'
                }`}
            >
              <div className="flex items-center gap-5">
                ...
                <div className="hidden sm:flex w-12 h-12 bg-white/5 rounded-full items-center justify-center border border-white/5">
                  <Briefcase size={20} className="text-[#ff4d94]" />
                </div>
                <div>
...
function EducationManager() {
  ...
  const fetchContent = async () => {
    try {
      ...
      const eduItem = dbData.find((item: any) => item.key === 'education_list');

      if (eduItem && eduItem.value) {
        setEducationList(JSON.parse(eduItem.value));
      } else {
        setEducationList([
          {
            degree: "Bachelor of Technology (B.Tech)",
            college: "Koneru Lakshmaiah Educational Foundation",
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await api.put('/content/education_list', { value: JSON.stringify(educationList) });
      alert('Education settings saved successfully!');
    } catch (err) {
      console.error("Error saving education settings:", err);
  ...
  const handleRemoveDegree = (idx: number) => {
    if (confirm('Remove this degree from your timeline?')) {
      setEducationList(educationList.filter((_, i) => i !== idx));
    }
  };

  const handleDegreeFieldChange = (idx: number, field: string, val: any) => {
    const updated = [...educationList];
    updated[idx][field] = val;
    setEducationList(updated);
  };

  const handleMoveDegree = (idx: number, direction: 'up' | 'down') => {
    const updated = [...educationList];
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center mb-2">
        <PageHeader title="Education & Academic Settings" />
      </div>

      <form onSubmit={handleSave} className="space-y-8 max-w-4xl">

        {/* Card 1: Degrees Timeline list */}
        <div className="bg-neutral-900/40 border border-white/5 p-6 sm:p-8 rounded-3xl backdrop-blur-md space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/5 pb-4">
            <div>
              <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] font-bold">Academic Timeline & Degree Specifics</h3>
              <p className="text-[10px] text-neutral-500 font-mono mt-1">Configure your educational degrees, specific academic focus areas, and coursework tags for each degree below.</p>
            </div>
            <button
              type="button"
              onClick={handleAddDegree}
              className="flex items-center gap-2 bg-[#ff4d94] text-black px-4 py-2 rounded-xl text-xs font-bold hover:bg-[#e5bf45] transition-all hover:scale-[1.02] active:scale-95 cursor-pointer"
            >
              <Plus size={14} /> Add New Degree
            </button>
          </div>

          {educationList.length === 0 ? (
            <p className="text-xs text-neutral-500 font-mono italic">No degrees added yet. Click 'Add New Degree' to start.</p>
          ) : (
            <div className="space-y-6">
              {educationList.map((edu, idx) => (
                <div key={idx} className="bg-black/30 border border-white/5 p-5 rounded-2xl space-y-5">
                  <div className="flex justify-between items-center pb-2 border-b border-white/5">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-1 bg-white/5 hover:bg-white/10 rounded text-neutral-400 hover:text-white disabled:opacity-30"
                      >
                        <ChevronUp size={14} />
                      </button>
                      <button
                      ...
                      <span className="text-xs font-mono text-[#ff4d94] uppercase tracking-widest font-bold">Degree #{idx + 1}: {edu.degree || 'New Degree'}</span>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <TextInput label="Degree / Course Title" value={edu.degree} onChange={(v: string) => handleDegreeFieldChange(idx, 'degree', v)} required />
                    <TextInput label="College / University Name" value={edu.college || ''} onChange={(v: string) => handleDegreeFieldChange(idx, 'college', v)} />
                    <TextInput label="Branch / Stream / Specialization" value={edu.institution} onChange={(v: string) => handleDegreeFieldChange(idx, 'institution', v)} required />
                    <TextInput label="Period (e.g. 2021 - 2025)" value={edu.period} onChange={(v: string) => handleDegreeFieldChange(idx, 'period', v)} required />
                    <TextInput label="Grade / GPA (Optional)" value={edu.grade || ''} onChange={(v: string) => handleDegreeFieldChange(idx, 'grade', v)} />

                    <div className="md:col-span-2">
                      <ArrayInput label="Key Highlights / Details" placeholder="e.g. Specialized in Software Engineering" value={edu.details} onChange={(v: any) => handleDegreeFieldChange(idx, 'details', v)} />
                    </div>

                    {/* Degree-Specific Academic Focus Areas */}
                    <div className="md:col-span-2 border-t border-white/5 pt-4 space-y-4">
                      <div className="flex justify-between items-center">
                        <h4 className="text-xs font-mono uppercase tracking-widest text-[#ff4d94] font-bold">Academic Focus Areas (Specific to this Degree)</h4>
                        <button
                          type="button"
                          onClick={() => {
                            const updated = [...educationList];
                            if (!updated[idx].academicFocus) updated[idx].academicFocus = [];
                            updated[idx].academicFocus.push({ title: '', desc: '' });
                            setEducationList(updated);
                          }}
                          className="text-[9px] font-mono uppercase tracking-widest bg-white/5 border border-white/5 hover:border-white/10 hover:text-white px-2.5 py-1.5 rounded-lg text-neutral-400 flex items-center gap-1"
                        >
                          <Plus size={10} /> Add Focus Area
                        </button>
                      </div>

                      {(!edu.academicFocus || edu.academicFocus.length === 0) ? (
                        <p className="text-[10px] text-neutral-500 font-mono italic pl-2">No focus areas added for this degree.</p>
                      ) : (
                        <div className="space-y-3">
                          {edu.academicFocus.map((focus: any, fIdx: number) => (
                            <div key={fIdx} className="bg-black/20 border border-white/5 p-3 rounded-xl flex items-start gap-3 animate-fade-in">
                              <div className="grow grid grid-cols-1 md:grid-cols-2 gap-3">
                                <TextInput
                                  ...
                                  onChange={(v: string) => {
                                    const updated = [...educationList];
                                    updated[idx].academicFocus[fIdx].title = v;
                                    setEducationList(updated);
                                  }}
                                  required
                                />
                                <TextInput
                                  ...
                                  onChange={(v: string) => {
                                    const updated = [...educationList];
                                    updated[idx].academicFocus[fIdx].desc = v;
                                    setEducationList(updated);
                                  }}
                                  required
                                />
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const updated = [...educationList];
                                  updated[idx].academicFocus = updated[idx].academicFocus.filter((_: any, i: number) => i !== fIdx);
                                  setEducationList(updated);
                                }}
                                className="p-2 bg-red-500/10 text-red-400 hover:bg-red-500/20 rounded-lg mt-5"
                              >
                                <Trash2 size={12} />
                              </button>
                            </div>
                    ...
                    {/* Degree-Specific Coursework Tags */}
                    <div className="md:col-span-2 border-t border-white/5 pt-4">
                      <ArrayInput
                        label="Coursework Tags (Specific to this Degree)"
                        placeholder="e.g. Data Structures"
                        value={edu.courseworkTags || []}
                        onChange={(v: string[]) => {
                          const updated = [...educationList];
                          updated[idx].courseworkTags = v;
                          setEducationList(updated);
                        }}
                      />
                    </div>
        ...
        <div className="flex justify-end pt-4">
          <button
            type="submit"
            disabled={saving}
            className="flex items-center gap-2 bg-[#ff4d94] text-black px-8 py-3.5 rounded-2xl font-bold hover:bg-[#e5bf45] transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50 cursor-pointer"
          >
            {saving ? <Loader2 className="animate-spin" size={18} /> : 'Save Education Settings'}
          </button>
        </div>
      </form>
    </div>
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingId ? 'Edit Certificate' : 'Add Certificate'}>
        <form onSubmit={handleSave} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              <label className="text-[10px] text-neutral-400 mb-1.5 font-mono uppercase tracking-wider pl-1">Description</label>
              <textarea className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white text-sm h-28 focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none transition-all custom-scrollbar" placeholder="Description of what you learned/accomplished..." value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} required />
            </div>
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-white/5 mt-6">
            <button type="button" onClick={closeModal} className="px-5 py-2.5 rounded-xl font-medium text-neutral-400 hover:text-white hover:bg-white/5 transition-colors">Cancel</button>
            <button type="submit" className="bg-[#ff4d94] text-black px-6 py-2.5 rounded-xl font-bold hover:bg-[#e5bf45] transition-colors">{editingId ? 'Save' : 'Add Certificate'}</button>
          </div>
        </form>
      </Modal>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#ff4d94] mt-1">{p.issuer} • {p.year}</p>
                </div>
              </div>
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => openModal(p)} className="p-2 bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 hover:text-blue-300 rounded-lg"><Edit2 size={16} /></button>
...
function HeroSettingsManager() {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSave} className="space-y-8 max-w-5xl bg-neutral-900/40 border border-white/5 p-6 sm:p-8 rounded-3xl backdrop-blur-md">

        {/* Headings */}
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Hero Text Content</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white text-sm h-28 focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none transition-all custom-scrollbar"
        ...
        {/* Dynamic Ticker */}
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Moving Background Tickers</h3>
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Call to Action Links</h3>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            ...
            <TextInput label="Email Address" value={data.hero_email_url} onChange={(v: any) => setData({ ...data, hero_email_url: v })} required />
          </div>
        </div>

        {/* Menu Drawer Decorative Media */}
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Menu Drawer Decorative Orb Media</h3>
        ...
        <div className="pt-6 border-t border-white/5 flex justify-end">
          <button
            type="submit"
            disabled={saving}
            className="flex items-center gap-2 bg-[#ff4d94] text-black px-8 py-3.5 rounded-2xl font-bold hover:bg-[#e5bf45] transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50"
          >
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSave} className="space-y-8 max-w-5xl bg-neutral-900/40 border border-white/5 p-6 sm:p-8 rounded-3xl backdrop-blur-md">
        ...
        {/* Intro */}
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Intro Settings</h3>
        ...
        {/* Highlights */}
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Profile Highlights</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-[#ff4d94] text-black px-6 py-3 rounded-xl font-bold hover:bg-[#e5bf45] transition-all hover:scale-[1.02] active:scale-95"
        >
...
function TogglesManager() {
  ...
  const [sectionOrder, setSectionOrder] = useState<string[]>([
    "about", "experience", "skills", "projects", "education", "certifications", "achievements", "game", "contact"
  ]);
  const [loading, setLoading] = useState(true);
  ...
  const fetchToggles = async () => {
    try {
      ...
      let orderVal: string | null = null;
      data.forEach((item: any) => {
        if (item.key.startsWith('toggle_')) {
          newToggles[item.key] = item.value === 'true';
        }
        if (item.key === 'section_order') {
          orderVal = item.value;
        }
      });
      setToggles(newToggles);
      if (orderVal) {
        const defaultOrder = ["about", "experience", "skills", "projects", "education", "certifications", "achievements", "game", "contact"];
        let loadedOrder = orderVal.split(',').map((s: string) => s.trim());
        loadedOrder = loadedOrder.filter(item => defaultOrder.includes(item));
        defaultOrder.forEach(item => {
          if (!loadedOrder.includes(item)) {
            loadedOrder.push(item);
          }
        });
  ...
  const handleMoveSection = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      await api.put('/content/section_order', { value: updated.join(',') });
    } catch (err) {
      console.error("Error saving section order:", err);
      alert('Failed to save section sequence.');
    }
  };
  ...
  const toggleItems = [
    ...
    { key: 'toggle_achievements', label: 'Achievements Section', description: 'Show or hide the achievements bento grid and leadership section.' },
  ];

  const sectionLabels: Record<string, string> = {
    about: "About (includes Education & Skills)",
    experience: "Experience (Services)",
    projects: "Projects (Creations)",
    certifications: "Certifications (Insights)",
    achievements: "Achievements & Leadership",
    game: "Asteroid Dodger Game",
    ...
    <div>
      ...
      {loading ? (
        ...
        <>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-white/5">
                    <span className={`text-xs font-mono uppercase tracking-wider ${isEnabled ? 'text-[#ff4d94]' : 'text-neutral-500'}`}>
                      {isEnabled ? 'Active / Visible' : 'Hidden / Disabled'}
                    </span>
                    <button
                      onClick={() => handleToggleChange(item.key, isEnabled)}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-300 focus:outline-none ${isEnabled ? 'bg-[#ff4d94]' : 'bg-neutral-800'
                        }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-black transition-transform duration-300 ${isEnabled ? 'translate-x-6' : 'translate-x-1'
                          }`}
                      />
                    </button>
                  </div>
          ...
          {/* Reordering Block */}
          <div className="mt-12 bg-neutral-900/40 backdrop-blur-sm border border-white/5 p-6 sm:p-8 rounded-3xl">
            <h2 className="text-xl font-bold text-white mb-2">Section Sequence / Ordering</h2>
            <p className="text-sm text-neutral-400 mb-6 font-sans">
              Adjust the order in which sections are displayed on the main portfolio page. Use the arrow keys to move sections up or down. Changes are saved automatically.
            </p>
            <div className="space-y-3">
              {sectionOrder.map((sectionId, idx) => (
                <div
                  key={sectionId}
                  className="flex items-center justify-between bg-black/40 border border-white/5 p-4 rounded-xl hover:border-white/10 transition-all duration-300"
                >
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-xs text-neutral-500">#{idx + 1}</span>
                    <span className="font-semibold text-white">{sectionLabels[sectionId] || sectionId}</span>
                  </div>
                  <div className="flex gap-1.5">
                    <button
                      ...
                      className="flex items-center justify-center p-2 bg-white/5 hover:bg-white/10 border border-white/5 rounded-lg text-neutral-400 hover:text-white transition-colors disabled:opacity-30 disabled:hover:bg-white/5 disabled:cursor-not-allowed"
                    >
                      <ChevronUp size={16} />
                    </button>
                    <button
...
function ChatbotSettingsManager() {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSave} className="space-y-8 max-w-5xl bg-neutral-900/40 border border-white/5 p-6 sm:p-8 rounded-3xl backdrop-blur-md">
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Custom AI Knowledge Base</h3>
          <p className="text-xs text-neutral-400 font-sans leading-relaxed">
          ...
          <div className="space-y-2">
            ...
            <textarea
              className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white text-sm h-64 focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none transition-all custom-scrollbar"
...
function ContactSettingsManager() {
  ...
  const defaultContactData = {
    ...
    contact_email: 'chitturianjana@gmail.com',
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSave} className="space-y-6 max-w-5xl bg-neutral-900/40 border border-white/5 p-6 sm:p-8 rounded-3xl backdrop-blur-md">

        {/* Availability Card settings */}
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Availability Dashboard</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="md:col-span-2">
              <SelectInput
        ...
        {/* Direct Coordinates */}
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Direct Contact Channels</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Consultations Header Block</h3>
          <div className="grid grid-cols-1 gap-5">
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-mono uppercase tracking-widest text-[#ff4d94] border-b border-white/5 pb-2">Recruiter Statement (Footer CTA Block)</h3>
          <div className="grid grid-cols-1 gap-5">
...
function AchievementsManager() {
  ...
  const defaultForm = {
    ...
    stats: [],
    accentColor: '#ff4d94',
    shadowColor: 'rgba(255, 77, 148, 0.12)',
    order: 0
  };
  const [formData, setFormData] = useState(defaultForm);
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingId ? 'Edit Achievement' : 'Create Achievement'}>
        <form onSubmit={handleSave} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <TextInput label="Accent Color (e.g. #ff4d94)" value={formData.accentColor} onChange={(v: any) => setFormData({ ...formData, accentColor: v })} required />
            <TextInput label="Shadow Glow Color (e.g. rgba(255, 77, 148, 0.12))" value={formData.shadowColor} onChange={(v: any) => setFormData({ ...formData, shadowColor: v })} required />
          ...
          <div className="flex flex-col">
            ...
            <textarea className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white text-sm h-24 focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none transition-all custom-scrollbar" placeholder="Short description..." value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} required />
          </div>

          <div className="flex flex-col">
            <label className="text-[10px] text-neutral-400 mb-1.5 font-mono uppercase tracking-wider pl-1">Experience & Impact</label>
            <textarea className="w-full bg-black/50 border border-white/10 p-4 rounded-xl text-white text-sm h-24 focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none transition-all custom-scrollbar" placeholder="Experience & impact details..." value={formData.impact} onChange={e => setFormData({ ...formData, impact: e.target.value })} required />
          </div>

          <div className="pt-4 flex justify-end gap-3 border-t border-white/5 mt-6">
            <button type="button" onClick={closeModal} className="px-5 py-2.5 rounded-xl font-medium text-neutral-400 hover:text-white hover:bg-white/5 transition-colors">Cancel</button>
            <button type="submit" className="bg-[#ff4d94] text-black px-6 py-2.5 rounded-xl font-bold hover:bg-[#e5bf45] transition-colors">{editingId ? 'Save Changes' : 'Publish Achievement'}</button>
          </div>
        </form>
      </Modal>
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          setError('Failed to log in automatically after signup. Please sign in manually.');
          setIsSignUp(false);
        }
      }, 1000);
    } catch (err: any) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#ff4d94]/10 blur-[120px] rounded-full pointer-events-none" />
        ...
        <div className="max-w-md w-full bg-neutral-900/60 backdrop-blur-xl border border-white/10 p-8 sm:p-10 rounded-4xl shadow-2xl relative z-10">
          <div className="flex flex-col items-center mb-8">
            <div className="w-12 h-12 bg-[#ff4d94]/10 border border-[#ff4d94]/20 rounded-xl flex items-center justify-center mb-4">
              <LayoutDashboard className="text-[#ff4d94]" size={24} />
            </div>
            <h2 className="text-2xl font-syne font-bold text-white tracking-wide">
              {isSignUp ? 'Create Admin Account' : 'CMS Access'}
            </h2>
            <p className="text-sm text-neutral-400 mt-2">
              {isSignUp ? 'Register as an administrator to manage content.' : 'Sign in to manage your portfolio.'}
            </p>
          </div>
          ...
          {success && <div className="mb-6 bg-green-500/10 border border-green-500/20 text-green-400 text-sm p-3 rounded-xl text-center">{success}</div>}
          ...
          <form onSubmit={isSignUp ? handleSignUp : handleLogin} className="space-y-5">
            <div>
              <input
                ...
                className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3.5 text-white text-sm focus:border-[#ff4d94]/50 focus:bg-white/5 focus:outline-none transition-all"
                required
              />
            </div>
            <div>
            ...
            <button
              type="submit"
              className="w-full bg-[#ff4d94] text-black font-bold py-3.5 rounded-xl hover:bg-[#e5bf45] hover:shadow-[0_0_20px_rgba(255, 77, 148,0.3)] transition-all mt-4 cursor-pointer"
            >
          ...
          <div className="mt-6 text-center">
            <button
              ...
              className="text-xs text-neutral-400 hover:text-[#ff4d94] transition-colors focus:outline-none cursor-pointer"
            >
              {isSignUp ? 'Already have an admin account? Sign In' : 'Need a new admin account? Sign Up'}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="h-screen bg-black text-white flex overflow-hidden font-sans">
      ...
      <aside className="w-72 bg-neutral-900/40 backdrop-blur-xl border-r border-white/5 flex flex-col relative z-20">
        <div className="p-8 pb-6 border-b border-white/5">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-8 h-8 bg-[#ff4d94] rounded-lg flex items-center justify-center shadow-[0_0_15px_rgba(255, 77, 148,0.4)]">
              <span className="text-black font-black font-syne text-lg">A</span>
            </div>
            <h2 className="text-xl font-syne font-bold tracking-wider text-white">Anjana</h2>
          </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 font-medium text-sm
                  ${isActive
                    ? 'bg-white/10 text-white shadow-[inset_2px_0_0_0_#ff4d94]'
                    : 'text-neutral-400 hover:bg-white/5 hover:text-white'
                  }`}
              >
                <div className={`${isActive ? 'text-[#ff4d94]' : 'text-neutral-500'}`}>
                  {link.icon}
                </div>
                {link.label}
              </Link>
      ...
      <main className="flex-1 relative z-10 h-screen overflow-y-auto custom-scrollbar bg-linear-to-br from-black via-neutral-950 to-black">
        ...
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#ff4d94]/5 blur-[150px] rounded-full pointer-events-none" />

        <div className="max-w-6xl w-full mx-auto p-8 md:p-12 relative z-10 min-h-full">
          <Routes>
            <Route path="/" element={
              <div className="flex flex-col items-center justify-center h-[70vh] text-center max-w-lg mx-auto animate-fade-in">
                <div className="w-20 h-20 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center mb-6 shadow-2xl">
                  <LayoutDashboard className="text-[#ff4d94] w-10 h-10" />
                </div>
