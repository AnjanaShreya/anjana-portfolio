      ...
      >
        {doubled.map((item, idx) => (
          <motion.div
            ...
            className="group flex items-center gap-4 md:gap-6 transition-all duration-300 opacity-[0.06] hover:opacity-[0.15] select-none text-white blur-[1px]"
          >
            {/* Logo container */}
            <div
              style={{ color: item.color }}
              className="transition-all duration-300 shrink-0 flex items-center justify-center text-[28px] md:text-[45px]"
            >
              {item.icon}
            </div>
            ...
            <span
              ...
              className="font-syne font-black tracking-tight uppercase whitespace-nowrap transition-all duration-300"
            >
              {item.name}
            </span>
          </motion.div>
...
const DEFAULT_CATEGORIES = [
  ...
  },
  {
    ...
    skills: [
      ...
      { name: "Express.js", color: "#E0E0E0", iconName: "express" }
    ]
  },
  {
    id: "database",
  ...
  },
  {
    ...
    iconName: "shield",
    skills: [
      { name: "Solidity", color: "#AAAEB0", iconName: "solidity" },
      { name: "Ethereum", color: "#627EEA", iconName: "ethereum" },
      { name: "Hardhat", color: "#FFF100", iconName: "hardhat" },
      { name: "MetaMask", color: "#F6851B", iconName: "metamask" }
    ]
  },
  {
    id: "ai",
    ...
    iconName: "cpu",
    skills: [
      { name: "OpenAI", color: "#10A37F", iconName: "openai" }
    ]
  },
  {
    id: "tools",
...
const mapSkillsWithIcons = (cats: any[]) => {
...
export default function SkillsSection() {
  ...
  const [categories, setCategories] = useState<any[]>(mapSkillsWithIcons(DEFAULT_CATEGORIES));
      ...
      .then((data: any[]) => {
        if (Array.isArray(data)) {
          ...
          if (skillsItem && skillsItem.value) {
            ...
            const withIcons = mapSkillsWithIcons(parsed);
            setCategories(withIcons);
            if (withIcons.length > 0) {
              if (!withIcons.some((c: any) => c.id === activeCategory)) {
  ...
  return (
    <div className="relative w-full overflow-hidden mt-2 min-h-[460px] flex flex-col items-center justify-center py-4 md:py-6">
        ...
        <div className="relative z-20 w-full max-w-3xl px-4">
          ...
          >
            ...
            <div className="text-center space-y-2 mb-8 md:mb-10">
              <span className="font-mono text-[9px] text-[#ff1493] tracking-widest uppercase font-semibold block">// STACK SHOWCASE</span>
              <h3 className="font-syne font-extrabold text-2xl md:text-3.5xl text-white tracking-tight">
                Engineering Toolkit
              </h3>
              <div className="h-px w-12 bg-[#ff1493]/30 mx-auto mt-2" />
            </div>
              ...
              {categories.map((category) => {
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center py-3 px-4 w-[100px] rounded-xl border text-center transition-all duration-300 cursor-pointer ${isActive
                        ? "border-[#ff1493] bg-[#ff1493]/5 text-[#ff1493] shadow-[0_0_12px_rgba(255,20,147,0.15)]"
                        : "border-white/5 bg-white/[0.02] text-neutral-400 hover:text-white hover:border-white/15"
                      }`}
                  >
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-3 md:gap-4 pt-2">
                      {activeCatData.skills.map((skill: any, idx: number) => (
                        <motion.div
                          ...
                          className="group flex items-center gap-2.5 font-mono text-xs md:text-sm px-4 py-2.5 rounded-full border border-white/5 bg-white/5 text-neutral-200 hover:border-white/25 hover:bg-white/[0.08] transition-all duration-300 cursor-default"
                        >
                          <div
                            style={{ color: skill.color }}
                            className="transition-all duration-300 text-[20px] md:text-[26px] flex items-center justify-center group-hover:scale-105"
                          >
                            {skill.icon}
                          </div>
                          <span className="group-hover:text-white transition-colors duration-300 font-semibold">{skill.name}</span>
                        </motion.div>
                      ))}
                    </div>
                  </motion.div>
            ...
            <div className="flex justify-end mt-6 md:mt-8">
              ...
              >
                <span className="text-xs sm:text-sm md:text-base font-bold text-neutral-300 group-hover:text-[#ff1493] transition-all duration-300">
                  Want to play a game
                </span>
                <span className="flex items-center gap-1.5 text-xs sm:text-sm md:text-base font-bold text-[#ff1493] underline decoration-[#ff1493]/40 group-hover:decoration-[#ff1493] group-hover:drop-shadow-[0_0_10px_rgba(255,20,147,0.4)] transition-all duration-300">
                  Click here
          ...
          <AnimatePresence>
            {isMemoryOpen && (
              <div className="fixed inset-0 z-50 flex justify-center items-start overflow-y-auto py-10 px-4 md:py-16 bg-black/85 backdrop-blur-xl">
                <div className="relative w-full max-w-xl bg-neutral-950 border border-white/10 p-4 sm:p-6 md:p-8 rounded-3xl my-auto shadow-[0_0_50px_rgba(255,20,147,0.15)]">
                  <button
